// Ported from cros-libva's lib/src/display.rs. `create_surfaces` and `create_context` are
// adjusted for `Surface` no longer being generic over a memory descriptor (see the crate-level
...
impl Display {
    ...
    ///
    /// `num_surfaces` surfaces will be created, each with its own `vaCreateSurfaces` call (in
    /// order to keep each `Surface` independently destroyable).
    ///
    /// # Arguments
    ///
    /// * `rt_format` - The desired surface format. See `VA_RT_FORMAT_*`
    /// * `va_fourcc` - The desired pixel format (optional). See `VA_FOURCC_*`
    /// * `width` - Width for the created surfaces
