// Ported from cros-libva's lib/src/picture.rs. `Picture<S, T>` keeps its `T` (container-of-Surface)
// generic for flexibility (e.g. a surface-pool wrapper type), but every `T: Borrow<Surface<D>>`
// bound loses its `D` generic since `Surface` is no longer generic (see the crate-level docs for
// the two deliberate deviations from cros-libva).
...
///
/// The `T` generic parameter must be `Borrow<Surface>`, i.e. it can be [`Surface`] directly or
...
impl<T> Picture<PictureNew, T> {
    ...
    pub fn new(timestamp: u64, context: Rc<Context>, surface: T) -> Self {
        Self {
    ...
    pub fn begin(self) -> Result<Picture<PictureBegin, T>, VaError>
    where
        T: Borrow<Surface>,
    {
...
impl<T> Picture<PictureEnd, T> {
    ...
    pub fn sync(self) -> Result<Picture<PictureSync, T>, (VaError, Self)>
    where
        T: Borrow<Surface>,
    {
...
impl<S: PictureReclaimableSurface, T> Picture<S, T> {
    ...
    pub fn derive_image<'a>(&'a self, visible_rect: (u32, u32)) -> Result<Image<'a>, VaError>
    where
        T: Borrow<Surface>,
    {
