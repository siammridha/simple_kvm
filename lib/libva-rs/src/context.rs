// Ported from cros-libva's lib/src/context.rs. `Context::new` drops the `SurfaceMemoryDescriptor`
// generic since `Surface` is no longer generic (see the crate-level docs for the two deliberate
