// Ported unchanged (in spirit) from cros-libva's lib/src/config.rs. `Config` does not depend on
// `Surface`, so it needed no changes for the non-generic-`Surface` deviation.
