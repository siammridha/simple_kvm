...
- **Settings are the engine's, in memory, never on disk.** `settings()` reads them, `update_settings(new)` applies them and fires a settings-changed event so every open tab updates live. A format is negotiated at open time, so changing settings under a running pass is impossible: the engine stops that pass and the pass's own supervisor starts the replacement once the old one has actually let go of the card. Startup defaults are a fixed combination here (720p@5fps) — `capture` has no probed capabilities of its own to fall back to any more (`rtc` is what now knows what the card actually supports, §3.4) — and are never overwritten once a person has picked settings by hand.
...
- **Owns exclusively:** the encode loop (driven from the `CaptureHandle`, sizing its buffers from that handle's negotiated resolution), the frame bus the encode pass publishes to, the `CaptureStream` type and its `ended` signal, and the current capture settings (held in memory) and their change event. The bus stays private; only `FrameEnvelope`, the frame a session pulls off a stream, is re-exported.
- **May depend on:** `device` (holds a `CaptureDevice`: `open` only — it doesn't subscribe to it any more).
- **Must not:** know the device path; read or write settings to disk; talk to `rtc`/`hid`/`web`; own peer sessions; compute `DeviceState` or track device presence/capabilities itself (moved to `rtc`, §3.4).
...
- **Presence updates the dropdown, but never by itself creates or destroys a video track.** `rtc`'s `devicechange` listener on its own `CaptureDevice` handle is what reacts to a plug/unplug. On a `Present` event, the payload already carries the probed `Info` (resolutions, frame rates) — `device` probed it before dispatching (§3.1), so `rtc` never makes a separate probe call. `rtc` recomputes `DeviceState` from that `Info` plus `capture.settings()` and pushes it down every session's `control` channel — that is all a `Present` event does on its own; it does **not** call `capture.request_stream()`, because a track nobody can receive isn't worth having (see the next bullet, and `capture`'s "encoder runs iff live handles > 0", §3.2/§6). On an `Absent` event, `rtc` recomputes `DeviceState` (`available: false`), pushes it the same way, then loops over every session currently holding the shared track (below), removing it and renegotiating, and drops its own reference to that shared stream.
- **One shared `CaptureStream`, handed to every peer — mirrors giving the same browser `MediaStreamTrack` to many `<video>` elements.** `rtc` holds at most one `CaptureStream` at a time, not one per session. When a session reaches `Connected` and the card is present: if `rtc` doesn't already hold a shared stream, it calls `capture.request_stream()` once to get one — this is the only place that call happens — and either way it then adds that same stream's track to the session (`add_track` + renegotiate); a session joining while a shared stream already exists just reuses it, with no second `request_stream()` call and no second device open. When a session ends, or otherwise stops holding the track, `rtc` checks whether any session still holds it; once none do, `rtc` drops its own reference, which lets `capture`'s live count reach zero and stop the encoder even though the card may still be plugged in — that is how "no session ⇒ no encoder" holds regardless of presence. The shared stream's own `ended` event (the pass dying after a successful start, §3.2) is handled the same way as the `Absent` case above: every session still holding the track loses it and renegotiates, and `rtc` drops its reference — whichever of `ended` or `Absent` happens first, the other is a no-op, since removing a track a session doesn't have does nothing.
...
| `rtc` | `device` | constructs its own `capture`/`hid` dependencies (`CaptureCard::spawn()`, `Hid::spawn()`); holds and subscribes to the `CaptureDevice`/`Ch9329Device` handles obtained from `capture.device()`/`hid.device()` for presence, probing, computing `DeviceState`/HID-available state, and tearing down the one shared video track across every session that holds it on an `Absent` event |
...
2. Capture card plugged → `CaptureDevice` probes → dispatches `devicechange(Present(info))`, which `rtc` hears directly (it holds its own clone of the same `CaptureDevice` handle, via `capture.device()` — §3.4). `rtc` recomputes `DeviceState` from that `info` (no separate probe call) and pushes it to every session's `control` channel — nothing else happens yet if no session is connected. The actual `capture.request_stream()` call, and the `Device::open` → `CaptureHandle` it triggers, only happens the moment a session reaches `Connected` while the card is present: the first such session causes the open and gets the resulting shared `CaptureStream`; every session after that — already connected or joining later — just gets that same shared stream added to it (`add_track` + renegotiate), with no further open.
...
6. Card unplugged → two independent things happen, in whatever order the hardware/OS delivers them, and either one is enough to leave every session correct: (a) `rtc`'s own `CaptureDevice` handle sees the unplug's `devicechange(Absent)` — it recomputes `DeviceState` (`available: false`), pushes it to every session's `control` channel, loops over every session still holding the shared track removing it and renegotiating, and drops its own reference to that stream; (b) the running encode pass's next read fails, ending the pass → the shared `CaptureStream` fires `ended` → `rtc` does the same removal loop and drops its reference. The encoder stops as soon as `rtc`'s reference to the shared stream is gone (`capture`'s live count reaches zero), and removing a track a session no longer has is a no-op, so the two paths racing is harmless. A session leaving normally (not from an unplug) is the same mechanism on a smaller scale: `rtc` removes the track from just that session, and if it was the last session holding it, drops the shared stream too — stopping the encoder even though the card is still present, since a card with no session watching it shouldn't be encoding.
