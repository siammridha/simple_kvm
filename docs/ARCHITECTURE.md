...
- **`open()` rechecks the real path, not just the cached status.** `is_present()`'s stored value can only be as fresh as the last uevent wake-up, so a device that disappeared after that but before this call would otherwise look present right up until `D::open` fails against it (or, worse, appears to succeed against a gone device). `open()` closes that gap with one extra, un-cached filesystem check of the device path immediately before calling `D::open` — both checks must agree the device is there. This is the generic `Device<D>::open`, so both the capture card and the CH9329 get it for free, not a per-device-kind fix.
...
- **Presence is noticed immediately; probing waits.** Real hardware (both the capture card and the CH9329) has crashed when probed/opened too soon after USB enumeration, including right at boot if the device was already plugged in. Rather than skipping the probe on just that first check, every absent→present transition is treated the same way: it's logged the instant it's seen, then the core waits a fixed `DETECT_TO_PROBE_DELAY` (5 seconds) before calling `D::probe` and dispatching `Present`. This is generic — one delay, applied uniformly to every `DeviceDriver`, not a per-device-kind workaround.
- **Probing rechecks the path after the delay, too.** The `DETECT_TO_PROBE_DELAY` wait means real time passes between noticing the device and actually probing it — long enough for a quick unplug to happen in between. Right before calling `D::probe`, the core checks the device path exists one more time; if it's gone, it skips the probe entirely (nothing was ever announced as `Present`, so there's nothing to retract) and resets its internal presence flag so the device's next real appearance is treated as a fresh detection rather than silently ignored.
- **No fallback polling for device presence.** Presence is learned exclusively from the kernel uevent stream — there is no periodic filesystem-poll substitute, ever. If the uevent listener itself fails to open, the presence task retries opening it a fixed number of times with a short delay between attempts; if every attempt fails, it logs an error and stops tracking that device rather than quietly degrading to a poll loop. A device whose uevent listener never opens reports no further presence changes until the process restarts.
...
- **Logging lives in the composition root, on purpose.** `main.rs` initialises `tracing` and prints the startup banner. Process-level observability is not domain logic, and the composition root *is* the process, so this is the one thing there that is neither construct, wire, nor start. It has no fallback of its own for a missing `RUST_LOG` — that's `EnvFilter::from_default_env()`'s own default (`error`-level only). The two real entry points each set `RUST_LOG` themselves: `deploy/install.sh` writes `/etc/conf.d/simple_kvm` with `RUST_LOG=info` for an installed service, and `test-on-device.sh` writes the same file with a debug-level filter for a test deploy. OpenRC sources `conf.d` automatically before the init script runs, so this is what actually controls a running service's verbosity.
