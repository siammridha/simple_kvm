...
- **`ended` is state, not only an edge.** A consumer gets its stream back and then does await-heavy work before it subscribes; a pass that fails fast (a bad open, a format the encoder can't use) ends the stream inside that window. So `ended` is published through a `StateEmitter` (§5.1): subscribing after the stream has already ended calls the listener straight away rather than registering one that can never fire. Without that, a session is stranded holding a video track for a pass that is already dead. A deliberate stop — the last stream dropping, so the live count hits zero — is still not `ended`, and there is still no retry or backoff: a dead pass stays dead until presence changes.
