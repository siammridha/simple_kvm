...
Investigation into replacing the software (`openh264`) encoder in `src/capture/h264.rs` with hardware encoding, to take load off the Atom x5-Z8350 CPU. This document is the record of what was tried, what didn't work and why, and what did work. No code from this investigation has been merged into the app yet - it all happened in a throwaway clone of a third-party library on the device itself.
...
The purpose-built wrapper mentioned in earlier drafts of this section now exists (`simple-kvm-vaapi`, see above) and the full pipeline has been ported onto it and verified. Remaining work:

- Nothing from this investigation has been merged into `src/` in this repo yet - it's all still living on the device in `/root/cros-codecs` and `/root/simple-kvm-vaapi`. Bringing the pipeline (and `simple-kvm-vaapi`) into this repo, replacing the `openh264` software encoder in `src/capture/h264.rs`, is the actual integration work.
- Still all-intra (IDR-only) encoding, the same limitation noted throughout this document. A real integration should use P-frames for efficiency; `simple-kvm-vaapi` doesn't currently need to change for that (the buffer types it wraps support it), but the pipeline code doesn't yet build reference-list/DPB management.
- `examples/raw_vaapi_encode.rs` and `examples/gpu_yuyv_to_h264.rs` (the `cros-libva`-based versions) remain as reference/comparison points; `examples/gpu_yuyv_to_h264_simple_vaapi.rs` is now the reference for the same flow on the crate intended for actual use.
