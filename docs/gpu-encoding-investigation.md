...
4. **Switched from CBR to VBR after real-world use showed jittery, pixelated output at 5 Mbps CBR that wasn't even an improvement over the old CPU encoder.** The old `openh264` CPU encoder never used a fixed bitrate target at all - it ran in its default "Quality" mode, sizing each frame by scene complexity rather than forcing a fixed number. Forcing the GPU encoder into strict CBR removed that flexibility: every frame gets squeezed into the same size regardless of complexity. The driver's supported rate-control modes were queried directly (`vaGetConfigAttributes` for `VAProfileH264ConstrainedBaseline`/`VAEntrypointEncSlice`) and confirmed to include VBR (bitmask `0x16` = `CBR | VBR | CQP`), so `src/capture/h264.rs` now negotiates `VA_RC_VBR` instead of `VA_RC_CBR`, with `target_percentage` set to 70 - the configured bitrate is now a peak/ceiling the encoder aims to average 70% of, rather than a number every single frame must hit exactly.

5. **2026-08-26: Switched from VBR to CQP - VBR still needed ~50 Mbps to look as smooth as the CPU encoder did under 1 Mbps, a huge efficiency gap traced to this hand-rolled encoder's own rate control/motion estimation under a bitrate target, not a hardware limit.** A separate `ffmpeg`-based GPU encoder (on another branch, same hardware) uses fixed-QP with no bitrate target at all and encodes cleanly at a bitrate similar to the CPU encoder - strong evidence the inefficiency was this encoder's own VBR handling, not the driver or hardware. This matches finding 2 above: before rate control was ever explicitly negotiated, this driver silently defaulted to constant-QP behavior on its own. `src/capture/h264.rs` now negotiates `VA_RC_CQP` instead of `VA_RC_VBR` explicitly, making that mode intentional rather than an accidental fallback. The fixed QP is unchanged - it was already `PIC_INIT_QP` (26), already wired into both the hand-packed PPS's `pic_init_qp_minus26` and `EncPictureParameterBufferH264`, and `slice_qp_delta = 0` in the hand-packed slice header - so no new QP plumbing was needed, only removing the VBR-specific `EncMiscParameterRateControl` buffer submission and the `bitrate` value/setting that fed it (which no longer exists anywhere in the app now that there's no bitrate concept for CQP to use). `EncSequenceParameterBufferH264::new`'s `bit_rate` argument is now passed as `0` - that field only feeds the driver's own internal HRD bookkeeping, which CQP doesn't use, and VUI/HRD parameters were never signaled in the hand-packed SPS to begin with.

6. **2026-08-26: Found and fixed the real cause of P-frame bloat under CQP - `RefPicList0` in the slice parameters was left all-`invalid()` even on P slices, contradicting `num_ref_idx_l0_active_minus1 = 0` (which declares one active reference).** After switching to CQP (finding 5), real-world measurement at 1080p@10fps still showed ~9.6 Mbps average - dramatically more than both the CPU encoder (~340 Kbps) and the `ffmpeg`-based GPU encoder on the other branch (~226 Kbps) at the same fixed QP (26), despite decoding cleanly with real P-frames. `encode_nv12` in `src/capture/h264.rs` was already pointing `pic_param.ReferenceFrames[0]` at the correct previous reconstructed surface, but separately built `ref_pic_list_0`/`ref_pic_list_1` for the slice parameters as entirely `PictureH264::invalid()` placeholders, unconditionally, even on P slices. That's the list the hardware actually searches for motion estimation - handing it "no valid reference" while simultaneously declaring one active reference index gave the encoder nothing good to predict from, forcing expensive, near-intra-quality coding on most of each P-frame even though the bitstream was structurally valid. Fixed by populating `ref_pic_list_0[0]` with the same reference picture as `pic_param.ReferenceFrames[0]` whenever the current slice isn't an IDR. Result, measured the same way as finding 5: average bitrate at 1080p@10fps dropped from ~9.6 Mbps to **~208 Kbps** - a ~46x reduction, and now in line with the `ffmpeg` GPU encoder's ~226 Kbps at the same QP.

7. **2026-08-26: Found and fixed a green padding bar at the bottom of the picture at 1080p, unique to this hand-rolled encoder.** As noted earlier in this document ("the encoded output is 1920x1088, not 1920x1080"), the encoder rounds width/height up to whole macroblocks (16px), so at 1080p it actually encodes a 1088-tall frame - 8 rows taller than the real image. The hand-packed SPS set `frame_cropping_flag = 0`, on the assumption this padding was harmless. It wasn't: with no crop signaled, decoders display the full 1088 rows, and the bottom 8 rows are uninitialized picture data, which decodes as solid green (YUV (0,0,0)). This only showed up at 1080p (1080 isn't a multiple of 16; 720p is, so it never padded) and only on this encoder (both the CPU `openh264` encoder and the `ffmpeg`-based GPU encoder on the other branch build their own SPS and set cropping correctly). Fixed in `pack_sps` (`src/capture/h264.rs`) by computing `frame_crop_bottom_offset`/`frame_crop_right_offset` from the real width/height whenever they differ from the padded coded width/height, and setting `frame_cropping_flag = 1` in that case.

8. **2026-08-26: Confirmed, with a throwaway on-device test, that this driver never emits its own SPS/PPS at all - hand-packing them is not optional.** After finding 7, tested whether the hand-packed SPS/PPS/slice headers could be dropped in favor of the driver's own auto-generated ones, by omitting `VAConfigAttribEncPackedHeaders` from the encode `Config` entirely (not just setting it to `0`, which the driver itself warns is a deprecated way to say the same thing: `vaCreateConfig: setting the EncPackedHeaders attribute to zero to indicate that no packed headers will be used is deprecated`) and skipping the `VAEncPackedHeaderSequence`/`Picture`/`Slice` buffer submissions. Captured real encoded output and inspected it byte-by-byte: of 229 NAL units produced, **all 229 were video-frame slices (type 1 or 5) - zero SPS (type 7), zero PPS (type 8)**, confirmed with `ffmpeg -i ... -f null -` failing immediately on every frame with `non-existing PPS 0 referenced`. This driver does not write SPS/PPS through this encode API at all, regardless of whether packed headers are negotiated - not a corruption bug like the slice-header finding at the top of this document, but a flat absence of the header NALs a decoder needs to even start. Confirmed this isn't project-specific by checking how mature encoders handle it: FFmpeg's own `vaapi_encode.c` queries the same `VAConfigAttribEncPackedHeaders` capability and, whenever a driver reports support (this one does), *always* builds and submits its own packed headers too, with the comment "packed headers are always preferable because they are under our control. When absent, the driver is generating them and some features may not work." gstreamer-vaapi's H.264 encoder does the same. So real FFmpeg running on this same hardware would make the identical choice this codebase already makes. This chip (Intel Atom Cherry Trail / HD Graphics 400, `i965` driver) also has a separate, unresolved, unrelated corruption bug on the decode side (`intel/libva` issue #626, "extensive blockiness/pixelation" on H.264/H.265 playback) - not the cause of this finding, but evidence this specific old driver on this specific old chip is generally known to be unreliable. Conclusion: hand-packing SPS/PPS/slice headers is a hard requirement on this device, not a workaround to eventually remove. The test code for this (env-var-gated `H264_USE_DRIVER_HEADERS`) was not merged - see the git stash from this investigation if it's ever needed again.
